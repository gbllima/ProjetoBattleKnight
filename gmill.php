...
<div align="center">
<? 	 echo"<img src='".$imgs.$fimgs."b0.gif' width='75' height='100'><br />";
 echo"<table width='650'><tr><td> ".$buildings[0][8]."</td></tr></table>";?> 
  ...
  for ($i=0; $i<1; $i++)
	 ...
	 if (!(($res[0]>=$cost[0]*$mcost)&&($res[1]>=$cost[1]*$mcost)&&($res[2]>=$cost[2]*$mcost)&&($res[3]>=$cost[3]*$mcost)&&($res[4]>=$cost[4]*$mcost))) 
	  echo "<b>".$lang['upgrade']." ".$name[1]."  ".$lang['toLevel']." ".($land[$i]+1)."</b>";
	  	  ...
	  	  echo "<a class='q_link' target='_parent' href='build.php?town=".$town[0]."&b=".$buildings[0][0]."&subB=".$i."'>".$lang['upgrade']." ".$name[1]."  ".$lang['toLevel']." ".($land[$i]+1)."</a>";
