...
/*
* Verarbeiten aller Anfragen zum H”™ndler/ Inventar
*  
	...
	var sell_request = new Request.JSON({
		...
		onComplete: function(data) {
						if (data.accomplished) {
							...
							//brauchen wir vielleicht auch nochn neuen H”™ndler?
							if (shop != false)  $('dealer').set('html', data.dealer);
							
							//Inventar ”Ãberschreiben
							if(data.inventory){
	...
	var potion_request = new Request.JSON({
		...
		onComplete: function(data) {
						if (data.accomplished) {
							...
							//HP Bubble und evtl. Tr”™nkeleiste ersetzen
	...
	var dress_request = new Request.JSON({
		...
		onComplete: function(data) {
						if (data.accomplished) {
							...
							//Neuer H”™ndler da neues Vergleichsitem notwendig?
							if (data.dealer) {
								var dealer = $('dealer');
								if (dealer) 
	...
	var request = new Request.JSON({
		...
		onComplete: function(data) {
						if (data.accomplished) {
							...
							//Request konnte nicht ausgef”Ãhrt werden, das ist schade; das Spiel muss dem User aber sagen warum
							roar.alert(news_title, data.roar_message);
						}
		}
...
/*
 * Schaltet im H”™ndler die Nutzbarkeit eines Items aus um es verkaufen zu kˇœÕnnen, onClick wird sonst mit ausgelˇœÕst
 * 
 */
...
function howNotToBeSeen() {
	if(Browser.Engine.trident && Browser.Engine.version < 7) {
		//H”™ndler nachtr”™glich hover beibringen
...
function boostAttribute(attribute, sub) {
	...
	var boost_request = new Request.JSON({
		...
		onComplete: function(data) {
						if (data.accomplished) {
							//Attribute gesteigert; der Ritter f”Ãhlt sich auch gleich viel toller
