...
if (isset($_SESSION["user"][0], $_GET["town"], $_POST["taxes"]))
{
 ...
 {
  if ($town[8][11]) $lvl=$town[8][11]-1; else $lvl=-1;
  update_taxes($prod[0]."-".$prod[1]."-".$prod[2]."-".$prod[3]."-".abs($_POST["taxes"]), 100+$buildings[11][5][$lvl]-abs($_POST["taxes"]), $_GET["town"]);
  header("Location: hall.php?town=".$_GET["town"]);
